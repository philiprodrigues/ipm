...
namespace {
  ...
  public:
    ...
    const static size_type bytesOnEachReceive = 10;

    ipmReceiverImpl() : can_receive_(false) {}
...
BOOST_AUTO_TEST_CASE(StatusChecks)
{
  ...
  BOOST_REQUIRE_EXCEPTION(theReceiver.receive(ipmReceiver::noblock, ipmReceiverImpl::bytesOnEachReceive - 1),
			  dunedaq::ipm::UnexpectedNumberOfBytes, [&](dunedaq::ipm::UnexpectedNumberOfBytes) { return true; });			  
  

  theReceiver.sabotage_my_receiving_ability();
  BOOST_REQUIRE(!theReceiver.can_receive());

  BOOST_REQUIRE_EXCEPTION(theReceiver.receive(ipmReceiver::noblock), dunedaq::ipm::KnownStateForbidsReceive, [&](dunedaq::ipm::KnownStateForbidsReceive) { return true; });
